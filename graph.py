"""
This module imports the inf constant from the math module.
The inf represents the positive infinity. It is greater than any other number.
"""
...
class Node:
    ...
    heuristic_value : int
        Coresponds to the manhattan distance plus the distance from the
        inital node to the current node. Default value is -1
    distance_from_start
        Corresponds to the distance of the node from the initial node.
        Defaul value is -1
    neighbors : list
        A list with the nodes the current node is connected
    parent : Node
        Represents the parent-node of the current node. Default value is
        None
    ...
    has_neighbors(self) -> Boolean
        Check if the current node is connected with other nodes
        (return True).
    ...
    def add_neighboor(self, neighboor):
        """
        Add a new node to the neighboor list. In other words create a new
    ...
    def __gt__(self, other):
        ...
        greater value.
        First examine the heuristic value. If this value is the same for
        both nodes
    ...
    def __eq__(self, other):
        """
        Define if current node and other node are equal, checking their
        values.
        Parameters
        ----------
            other: Node:
...
class Graph:
    ...
    def get_node(self, node_id):
        """
        This function it belongs to the graph class and it is used to find
        """
    ...
    def are_connected(self, node_one, node_two):
        """
        Return True if the given nodes are connected. Otherwise return
        False
        ...
        Parameters
        ----------
